...
<table style="width:100%;">
	...
	for($i=0;$i<6;$i++) {
		...
		for($j=0;$j<7;$j++)
                {
                    echo '<td style="border:solid 1px red;height:200px;" '.(($num_mois == date("n") && $num_an == date("Y") && $tab_cal[$i][$j] == date("j"))?' style="color: #FFFFFF; background-color: #000000;"':null).">";
                    echo ((strpos($tab_cal[$i][$j],"*")!==false)?'<font color="#aaaaaa">'.str_replace("*","",$tab_cal[$i][$j]).'</font>':$tab_cal[$i][$j]);
                    echo "</td>";
		}
		echo "</tr>";
	}
